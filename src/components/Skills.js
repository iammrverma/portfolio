...
const Skill = ({ name, x, y }) => {
  return (
    <motion.div
      className="flex items-center justify-center rounded-full font-semibold bg-dark text-light py-3 px-6 shadow-dark cursor-pointer absolute uppercase dark:text-dark dark:bg-light
      lg:py-2 lg:px-4 md:text-sm md:py-1.5 md:px-3 
      xs:bg-transparent xs:text-dark xs:dark:bg-transparent
      xs:tex-dark xs:dark:text-light xs:font-bold"
    ...
    <>
      ...
      >
        <motion.div
          ...
          lg:p-6 md:p-4 md:text-sm  xs:text-xs xs:p-2"
          whileHover={{ scale: 1.05 }}
        >
          web
        </motion.div>
